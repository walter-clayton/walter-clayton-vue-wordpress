...
const twitterShareUrl = computed(() => {
  if (!post.value) return '';
  return `https://twitter.com/intent/tweet?url=${encodeURIComponent(`https://www.walterclayton.com/post/${post.value.slug}`)}&text=${encodeURIComponent(post.value.title.rendered)}`;
});

const facebookShareUrl = computed(() => {
  if (!post.value) return '';
  return `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://www.walterclayton.com/post/${post.value.slug}`)}`;
});

const linkedInShareUrl = computed(() => {
  if (!post.value) return '';
  return `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://www.walterclayton.com/post/${post.value.slug}`)}`;
});
